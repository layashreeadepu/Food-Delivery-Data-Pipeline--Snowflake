...
create or replace table staging_area.orders (
...
create or replace stream staging_area.orders_stm 
on table staging_area.orders
append_only = true
comment = 'This is the append-only stream object on orders entity that only gets delta data';

list  @staging_area.csv_stg/initial/orders/orders-initial.csv;


copy into staging_area.orders (orderid, customerid, restaurantid, orderdate, totalamount, 
...
from (
    ...
    from @staging_area.csv_stg/initial/orders/orders-initial.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;


CREATE OR REPLACE TABLE cleaning_area.ORDERS (
...
create or replace stream cleaning_area.ORDERS_stm 
on table cleaning_area.ORDERS
comment = 'This is the stream object on ORDERS table table to track insert, update, and delete changes';


MERGE INTO cleaning_area.ORDERS AS target
USING staging_area.ORDERS_STM AS source
...
-- part-2
list @staging_area.csv_stg/delta/orders/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @staging_area.csv_stg/delta/orders/day-02-orders.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;
