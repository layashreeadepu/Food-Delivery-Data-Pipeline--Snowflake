...
use schema consumption_area;


CREATE OR REPLACE TABLE consumption_area.order_item_fact (
...
MERGE INTO consumption_area.order_item_fact AS target
USING (
    SELECT 
    ...
    FROM 
        cleaning_area.order_item_stm oi
    JOIN 
        cleaning_area.orders_stm o ON oi.Order_ID_fk = o.Order_ID
    JOIN 
        cleaning_area.delivery_stm d ON o.Order_ID = d.Order_ID_fk
    JOIN 
        consumption_area.CUSTOMER_DIM c on o.Customer_ID_fk = c.customer_id
    JOIN 
        consumption_area.CUSTOMER_ADDRESS_DIM ca on c.Customer_ID = ca.CUSTOMER_ID_fk
    JOIN 
        consumption_area.restaurant_dim r on o.Restaurant_ID_fk = r.restaurant_id
    JOIN 
        consumption_area.menu_dim m ON oi.MENU_ID_fk = m.menu_id
    JOIN 
        consumption_area.delivery_agent_dim da ON d.Delivery_Agent_ID_fk = da.delivery_agent_id
    JOIN 
        consumption_area.restaurant_location_dim rl on r.LOCATION_ID_FK = rl.location_id
    JOIN 
        consumption_area.DATE_DIM dd on dd.calendar_date = date(o.order_date)
) AS source_stm
ON 
