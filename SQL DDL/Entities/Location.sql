...
use schema sandbox.staging_area;

-- drop table staging_area.location 
-- drop table CLEANING_AREA.RESTAURANT_LOCATION
-- drop table SANDBOX.CONSUMPTION_AREA.RESTAURANT_LOCATION_DIM
-- drop stream SANDBOX.STAGING_AREA.LOCATION_STM
-- drop stream SANDBOX.CLEANING_AREA.RESTAURANT_LOCATION_STM

create table staging_area.location (
    locationid text,
    city text,
    state text,
    zipcode text,
...
create or replace stream staging_area.location_stm 
on table staging_area.location
append_only = true
comment = 'this is the append-only stream object on location table that gets delta data based on changes';

copy into staging_area.location (locationid, city, state, zipcode, activeflag, 
...
from (
    ...
    from @staging_area.csv_stg/initial/location t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;



select * from staging_area.location;
select * from staging_area.location_stm;

use schema cleaning_area;

-- Level 2
create or replace table cleaning_area.restaurant_location (
...
create or replace stream cleaning_area.restaurant_location_stm 
on table cleaning_area.restaurant_location
comment = 'this is a standard stream object on the location table to track insert, update, and delete changes';


MERGE INTO cleaning_area.restaurant_location AS target
USING (
    SELECT 
        ...
        _stg_file_md5,
        CURRENT_TIMESTAMP AS _copy_data_ts
    FROM staging_area.location_stm
) AS source
...
create or replace table consumption_area.restaurant_location_dim (
...
MERGE INTO 
        consumption_area.RESTAURANT_LOCATION_DIM AS target
    USING 
        cleaning_area.RESTAURANT_LOCATION_STM AS source
    ON 
...
list @staging_area.csv_stg/delta/location;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @staging_area.csv_stg/delta/location/delta-day03-invalid-delimiter.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement
;
...
select * from consumption_area.restaurant_location_dim order by city;
