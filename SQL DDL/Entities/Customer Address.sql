...
create or replace table staging_area.customeraddress (
...
create or replace stream staging_area.customeraddress_stm 
on table staging_area.customeraddress
append_only = true
comment = 'This is the append-only stream object on customer address table that only gets delta data';


select * from staging_area.customeraddress_stm;
copy into staging_area.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
...
from (
    ...
    from @staging_area.csv_stg/delta/customer-address t
)
...
-- 2nd layer
CREATE OR REPLACE TABLE CLEANING_AREA.CUSTOMER_ADDRESS (
...
create or replace stream CLEANING_AREA.CUSTOMER_ADDRESS_STM
on table CLEANING_AREA.CUSTOMER_ADDRESS
comment = 'This is the stream object on customer address entity to track insert, update, and delete changes';


MERGE INTO CLEANING_AREA.customer_address AS clean
USING (
    SELECT 
        ...
        _stg_file_md5,
        _copy_data_ts
    FROM staging_area.customeraddress_stm 
) AS stage
...
CREATE OR REPLACE TABLE CONSUMPTION_AREA.CUSTOMER_ADDRESS_DIM (
...
-- select * from CLEANING_AREA.CUSTOMER_ADDRESS_STM;
MERGE INTO 
    CONSUMPTION_AREA.CUSTOMER_ADDRESS_DIM AS target
USING 
    CLEANING_AREA.CUSTOMER_ADDRESS_STM AS source
ON 
...
--
select * from staging_area.customeraddressbook;
select * from CLEANING_AREA.CUSTOMER_ADDRESS_BOOK;
select * from CONSUMPTION_AREA.CUSTOMER_ADDRESS_BOOK_DIM;


list @staging_area.csv_stg/delta/customer-address;
copy into staging_area.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
                               landmark, locality,city,pincode, state, coordinates, primaryflag, addresstype, 
                               createddate, modifieddate, 
                               _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
