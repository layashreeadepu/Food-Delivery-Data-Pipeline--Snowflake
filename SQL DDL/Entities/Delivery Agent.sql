...
create or replace table staging_area.deliveryagent (
...
create or replace stream staging_area.deliveryagent_stm 
on table staging_area.deliveryagent
append_only = true
comment = 'This is the append-only stream object on delivery agent table that only gets delta data';


copy into staging_area.deliveryagent (deliveryagentid, name, phone, vehicletype, locationid, 
...
from (
    ...
    from @staging_area.csv_stg/initial/delivery-agent t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;

select count(*) from staging_area.deliveryagent;

select * from staging_area.deliveryagent_stm;


CREATE OR REPLACE TABLE cleaning_area.delivery_agent (
...
create or replace stream cleaning_area.delivery_agent_stm 
on table cleaning_area.delivery_agent
comment = 'This is the stream object on delivery agent table table to track insert, update, and delete changes';


MERGE INTO cleaning_area.delivery_agent AS target
USING staging_area.deliveryagent_stm AS source
...
select * from cleaning_area.delivery_agent_stm ;


CREATE OR REPLACE TABLE consumption_area.delivery_agent_dim (
...
MERGE INTO consumption_area.delivery_agent_dim AS target
USING cleaning_area.delivery_agent_stm AS source
ON 
...
list @staging_area.csv_stg/delta/delivery-agent;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @staging_area.csv_stg/delta/delivery-agent/day-02-delivery-agent.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;

select * from consumption_area.delivery_agent_dim;
