...
create or replace table staging_area.customer (
...
create or replace stream staging_area.customer_stm 
on table staging_area.customer
append_only = true
...
copy into  staging_area.customer (customerid, name, mobile, email, loginbyusing, gender, dob, anniversary, 
...
from (
    ...
    from @staging_area.CSV_STG/initial/customer/customers-initial.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;

select * from staging_area.customer limit 10;
select count(*) from staging_area.customer; -- 99899
select count(*) from staging_area.customer_stm; 


-- Part-2 Clean Layer
-- 

CREATE OR REPLACE TABLE cleaning_area.CUSTOMER (
...
-- Stream object to capture the changes. 
create or replace stream cleaning_area.customer_stm 
on table cleaning_area.customer
comment = 'This is the stream object on customer entity to track insert, update, and delete changes';


insert into cleaning_area.customer (
    customer_id,
    name,
    mobile,
    email,
...
select 
    ...
    _stg_file_md5,
    _copy_data_ts
from staging_area.customer;


MERGE INTO cleaning_area.CUSTOMER AS target
USING (
    SELECT 
        ...
        _STG_FILE_MD5,
        _COPY_DATA_TS
    FROM staging_area.CUSTOMER_STM
) AS source
...
-- create dim table 
CREATE OR REPLACE TABLE CONSUMPTION_area.CUSTOMER_DIM (
...
MERGE INTO 
    CONSUMPTION_AREA.CUSTOMER_DIM AS target
USING 
    cleaning_area.CUSTOMER_STM AS source
ON 
...
list @staging_area.csv_stg/delta/customer/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @staging_area.csv_stg/delta/customer/day-02-insert-update.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;
