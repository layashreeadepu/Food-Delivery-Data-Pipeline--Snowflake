...
create or replace table staging_area.menu (
...
create or replace stream staging_area.menu_stm 
on table staging_area.menu
append_only = true
comment = 'This is the append-only stream object on menu entity that only gets delta data';


list @staging_area.csv_stg/initial/menu;

copy into staging_area.menu (menuid, restaurantid, itemname, description, price, category, 
...
create or replace stream cleaning_area.menu_stm 
on table cleaning_area.menu
comment = 'This is the stream object on menu table table to track insert, update, and delete changes';


MERGE INTO cleaning_area.menu AS target
USING (
    SELECT 
...
CREATE OR REPLACE TABLE consumption_area.menu_dim (
...
MERGE INTO 
    consumption_area.MENU_DIM AS target
USING 
    cleaning_area.MENU_STM AS source
ON 
...
-- Part -2
list @staging_area.csv_stg/delta/menu;
copy into staging_area.menu (menuid, restaurantid, itemname, description, price, category, 
                availability, itemtype, createddate, modifieddate,
                _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @staging_area.csv_stg/delta/menu/ t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;
