...
create or replace table staging_area.restaurant (
    restaurantid text,      
...
create or replace stream staging_area.restaurant_stm 
on table staging_area.restaurant
append_only = true
...
copy into staging_area.restaurant (restaurantid, name, cuisinetype, pricing_for_2, restaurant_phone, 
...
from (
    select 
     ...
     from @staging_area.csv_stg/initial/restaurant/restaurant-delhi+NCR.csv t
)
...
create or replace table cleaning_area.restaurant (
...
create or replace stream cleaning_area.restaurant_stm 
on table cleaning_area.restaurant
...
insert into cleaning_area.restaurant (
    restaurant_id,
    name,
    cuisine_type,
    pricing_for_two,
...
select 
    ...
    _stg_file_md5
from 
    staging_area.restaurant;
        
-- here is the merge statement
MERGE INTO cleaning_area.restaurant AS target
USING (
    SELECT 
        try_cast(restaurantid AS number) AS restaurant_id,
        try_cast(name AS string) AS name,
...
CREATE OR REPLACE TABLE consumption_area.RESTAURANT_DIM (
...
select count(*) from cleaning_area.RESTAURANT_STM;

-- merge statement
MERGE INTO 
    consumption_area.RESTAURANT_DIM AS target
USING 
    cleaning_area.RESTAURANT_STM AS source
ON 
...
list @staging_area.csv_stg/delta/restaurant/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @staging_area.csv_stg/delta/restaurant/day-02-upsert-restaurant-delhi+NCR.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;
