...
create or replace table staging_area.orderitem (
...
create or replace stream staging_area.orderitem_stm 
on table staging_area.orderitem
append_only = true
comment = 'This is the append-only stream object on order item table that only gets delta data';

list @staging_area.csv_stg/initial/order-item;

copy into staging_area.orderitem (orderitemid, orderid, menuid, quantity, price, 
...
from (
    ...
    from @staging_area.csv_stg/initial/order-item/order-Item-initial.csv t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;

select * from staging_area.orderitem;
select * from staging_area.orderitem_stm;

CREATE OR REPLACE TABLE cleaning_area.order_item (
...
create or replace stream cleaning_area.order_item_stm 
on table cleaning_area.order_item
comment = 'This is the stream object on order_item table table to track insert, update, and delete changes';


select * from cleaning_area.order_item_stm;


MERGE INTO cleaning_area.order_item AS target
USING staging_area.orderitem_stm AS source
ON  
...
-- part-2
list @staging_area.csv_stg/delta/order-items/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @staging_area.csv_stg/delta/order-items/ t
)
file_format = (format_name = 'staging_area.csv_file_format')
on_error = abort_statement;
