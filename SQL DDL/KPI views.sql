...
select * from consumption_area.order_item_fact limit 100;

create or replace view consumption_area.vw_yearly_revenue_kpis as
select
...
from
    consumption_area.order_item_fact fact
join
    consumption_area.date_dim d
on
...
CREATE OR REPLACE VIEW consumption_area.vw_monthly_revenue_kpis AS
SELECT
    d.YEAR AS year,                       -- Fetch year from DATE_DIM
...
CREATE OR REPLACE VIEW consumption_area.vw_daily_revenue_kpis AS
SELECT
    d.YEAR AS year,                       -- Fetch year from DATE_DIM
    d.MONTH AS month,                     -- Fetch month from DATE_DIM
...
CREATE OR REPLACE VIEW consumption_area.vw_day_revenue_kpis AS
SELECT
    d.YEAR AS year,                       -- Fetch year from DATE_DIM
    d.MONTH AS month,                     -- Fetch month from DATE_DIM
...
CREATE OR REPLACE VIEW consumption_area.vw_monthly_revenue_by_restaurant AS
SELECT
    d.YEAR AS year,                       -- Fetch year from DATE_DIM
    d.MONTH AS month,                     -- Fetch month from DATE_DIM
...
JOIN
    consumption_area.restaurant_dim r
ON
